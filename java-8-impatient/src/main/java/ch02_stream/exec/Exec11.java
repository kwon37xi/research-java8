 ...
 * 이 작업을 어떻게 할 수 있나?
 *
 * -> 문제의 의도를 마음대로 해석해보면, Stream<ArrayList<T>> 가 가진 모든 ArrayList 들의 size()의 총 합과 동일한 size의 ArrayList<T> all가 존재할 때
...
public class Exec11 {
    public static void main(String[] args) {
        ...
        final Stream<ArrayList<Integer>> stream = Stream.of(new ArrayList<>(Arrays.asList(1, 2, 3)), new ArrayList<>(Arrays.asList(4, 5, 6)), new ArrayList<>(Arrays.asList(7, 8, 9, 10)));

        ArrayList<Integer> all = new ArrayList<>(Arrays.asList(new Integer[10]));

        AtomicInteger indexer = new AtomicInteger();

        // indexer를 모든 값 단위로 increment
//        stream.parallel().forEach(integers -> integers.parallelStream().forEach(i -> all.set(indexer.getAndIncrement(), i)));

        // or indexer를 각 List 단위로 increment
        stream.parallel()
            .forEach(integers -> {
                final int parentIndex = indexer.getAndAdd(integers.size());
                final AtomicInteger localIndexer = new AtomicInteger();
                integers.parallelStream()
                    .forEach(i -> all.set(parentIndex + localIndexer.getAndIncrement(), i));
            });
